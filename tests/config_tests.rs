...
/// Guard to restore the working directory when dropped
struct DirGuard {
...
fn change_to_temp_dir() -> (tempfile::TempDir, DirGuard) {
    let temp_dir = tempfile::tempdir().unwrap();
    let original = std::env::current_dir().unwrap();
    std::env::set_current_dir(temp_dir.path()).unwrap();
    (temp_dir, DirGuard { original })
}
...
fn test_config_load_creates_default_when_missing() {
    ...
    assert_eq!(config.refresh_interval_seconds, 3);
